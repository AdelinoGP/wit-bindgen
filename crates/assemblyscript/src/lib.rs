...
use wit_bindgen_core::wit_parser::{
    ...
    InterfaceId, ManglingAndAbi, Record, Resolve, Result_, SizeAlign, Tuple, Type,
    TypeDefKind, TypeId, TypeOwner, Variant, WorldId, WorldKey,
};
use wit_bindgen_core::{
    AsyncFilterSet, Direction, Files, InterfaceGenerator as CoreInterfaceGenerator,
    WorldGenerator,
};
...
impl AssemblyScript {
                ...
                let pkg = iface
                    ...
                    .map(|p| {
                        ...
                        let ver = pname
                            ...
                            .map(|v| format!("@{v}"))
                            .unwrap_or_default();
                        format!("{}:{}/{}{ver}", pname.namespace, pname.name,
    ...
    fn render_bindings_ts(
        ...
        writeln!(s, "//").unwrap();
        writeln!(s, "// Entry point for `asc`. Wasm-export wrappers below dispatch into").unwrap();
        writeln!(s, "// user-edited files under `exports/`. The wrappers are emitted with").unwrap();
        writeln!(s, "// AS-valid identifier names; a post-compile step renames the wasm").unwrap();
        ...
        writeln!(s, "//").unwrap();
        writeln!(s, "// ASYNC LIMITATIONS: future<T>/stream<T>/error-context types are").unwrap();
        writeln!(s, "// emitted as opaque i32 handle aliases. Functions marked `async` in").unwrap();
        ...
        for basename in imports.keys() {
            writeln!(s, "import * as i_{basename} from \"./imports/{basename}\";")
                .unwrap();
        }
        for basename in exports.keys() {
            writeln!(s, "import * as e_{basename} from \"./exports/{basename}\";")
                .unwrap();
        }
        if !self.world_imports.src.is_empty() || !self.world_exports.src.is_empty() {
        ...
        for entry in &self.exports {
            by_basename.entry(entry.basename.as_str()).or_default().push(entry);
        }
    ...
    fn render_ffi_ts(&self) -> String {
        ...
        s.push('\n');
        writeln!(s, "// ---------------------------------------------------------------------------").unwrap();
        writeln!(s, "// World-specific return area. Sized at generation time to the maximum").unwrap();
        writeln!(s, "// canonical-ABI return area required by any single import in this world.").unwrap();
        writeln!(s, "// ---------------------------------------------------------------------------").unwrap();
        writeln!(s).unwrap();
        let size = self.import_return_area_size.max(8);
        let _align = self.import_return_area_align.max(4);
        writeln!(s, "@global export const __IMPORT_RETURN_AREA: usize = changetype<usize>(").unwrap();
...
fn render_interface_file(frag: &InterfaceFragment, kind: &str) -> String {
    ...
    for basename in &frag.imports_imports {
        writeln!(s, "import * as i_{basename} from \"{prefix}imports/{basename}\";").unwrap();
    }
    for basename in &frag.imports_exports {
        writeln!(s, "import * as e_{basename} from \"{prefix}exports/{basename}\";").unwrap();
    }
    writeln!(s).unwrap();
    s.push_str(&frag.src);
    s
    ...
    fn type_id_ref(&mut self, id: TypeId) -> String {
        ...
        match &ty.kind {
            ...
            | TypeDefKind::Handle(Handle::Borrow(resource_id)) => {
                self.named_type_ref(*resource_id)
            }
            ...
            TypeDefKind::Result(r) => {
                let ok = r.ok.map(|t| self.type_ref(&t)).unwrap_or_else(|| "i32".into());
                let err = r.err.map(|t| self.type_ref(&t)).unwrap_or_else(|| "i32".into());
                format!("ffi.Result<{ok}, {err}>")
            }
            TypeDefKind::Variant(_) | TypeDefKind::Record(_)
            | TypeDefKind::Enum(_) | TypeDefKind::Flags(_) | TypeDefKind::Resource => {
                self.named_type_ref(id)
            }
...
impl FuncSig {
    ...
    fn wasm_wrapper_signature(&self, name: &str) -> String {
        let ps: Vec<String> = self
            ...
            .map(|(i, t)| format!("a{i}: {}", wasm_type_name(*t)))
            .collect();
        let ret = self.wasm_return_type().unwrap_or_else(|| "void".to_string());
        format!("{name}({}): {ret}", ps.join(", "))
    }
...
impl<'a> CoreInterfaceGenerator<'a> for InterfaceGenerator<'a> {
    ...
    fn type_record(&mut self, _id: TypeId, name: &str, record: &Record, docs: &Docs) {
        ...
        let fields: Vec<(String, String, Docs)> = record
            .fields
            .iter()
            .map(|f| (Self::field_ident(&f.name), self.type_ref(&f.ty), f.docs.clone()))
            .collect();
    ...
    fn type_resource(&mut self, id: TypeId, name: &str, docs: &Docs) {
        ...
        match self.direction {
            ...
            Direction::Export => {
                ...
                writeln!(self.src, "const __{safe}_table: Map<i32, {cls}> = new Map<i32, {cls}>();").unwrap();
                writeln!(self.src, "let __{safe}_next: i32 = 1;").unwrap();
                writeln!(self.src, "export function __{safe}_take(inst: {cls}): i32 {{").unwrap();
                writeln!(self.src, "  const h = __{safe}_next++; __{safe}_table.set(h, inst); return h;").unwrap();
                writeln!(self.src, "}}").unwrap();
                ...
                writeln!(self.src, "  if (inst !== null) {{ inst.__onDrop(); __{safe}_table.delete(h); }}").unwrap();
                writeln!(self.src, "}}").unwrap();
                writeln!(self.src).unwrap();

                // Register the canonical wasm-export `[dtor]<resource>` wrapper.
                let key = self.interface_key.as_ref().expect(
                    "world-level exported resource without an enclosing interface",
                );
    ...
    fn type_flags(&mut self, _id: TypeId, name: &str, flags: &Flags, docs: &Docs) {
        ...
        writeln!(self.src, "export class {cls} {{").unwrap();
        writeln!(self.src, "  constructor(public bits: {underlying} = 0) {{}}").unwrap();
...
impl<'a, 'b> Bindgen for FunctionBindgen<'a, 'b> {
    ...
    fn emit(
        ...
        match inst {
            ...
            Instruction::I32Load { offset } => {
                results.push(format!("load<i32>({} + {})", operands[0], offset.size_wasm32()));
            }
            Instruction::I32Load8U { offset } => {
                results.push(format!("(<i32>load<u8>({} + {}))", operands[0], offset.size_wasm32()));
            }
            Instruction::I32Load8S { offset } => {
                results.push(format!("(<i32>load<i8>({} + {}))", operands[0], offset.size_wasm32()));
            }
            Instruction::I32Load16U { offset } => {
                results.push(format!("(<i32>load<u16>({} + {}))", operands[0], offset.size_wasm32()));
            }
            Instruction::I32Load16S { offset } => {
                results.push(format!("(<i32>load<i16>({} + {}))", operands[0], offset.size_wasm32()));
            }
            Instruction::I64Load { offset } => {
                results.push(format!("load<i64>({} + {})", operands[0], offset.size_wasm32()));
            }
            Instruction::F32Load { offset } => {
                results.push(format!("load<f32>({} + {})", operands[0], offset.size_wasm32()));
            }
            Instruction::F64Load { offset } => {
                results.push(format!("load<f64>({} + {})", operands[0], offset.size_wasm32()));
            }
            Instruction::PointerLoad { offset } => {
                results.push(format!("load<usize>({} + {})", operands[0], offset.size_wasm32()));
            }
            ...
            | Instruction::I32FromS32 => results.push(format!("(<i32>{})", operands[0])),
            ...
            Instruction::StringLower { realloc } => {
                ...
                if realloc.is_some() {
                    ...
                    let buf = self.fresh();
                    self.push_line(&format!(
                        "const {bytes} = (<usize>{s_ref}.length) << 1;"
                    ));
                    self.push_line(&format!(
                    ...
                    self.push_line(&format!(
                        "const {s_ptr} = changetype<usize>({s_ref});"
                    ));
                    self.push_line(&format!("const {s_len} = <i32>{s_ref}.length;"));
                }
                results.push(s_ptr);
            ...
            Instruction::ListLift { element, .. } => {
                ...
                self.push_line(&format!(
                    "const {arr} = new Array<{elem_ty}>({len});"
                ));
                let (block_src, block_results) =
            ...
            Instruction::HandleLower { handle, .. } => {
                ...
                if self.iface_gen.world_gen.exported_resources.contains(&res_id) {
            ...
            Instruction::VariantLower { variant, results: wasm_results, ty, .. } => {
            ...
            Instruction::OptionLower { payload, results: wasm_results, .. } => {
                ...
                let (some_src, some_res) = self.blocks.pop().expect("OptionLower expects Some block");
                let (none_src, none_res) = self.blocks.pop().expect("OptionLower expects None block");
                let _ = payload;
            ...
            Instruction::OptionLift { payload, .. } => {
                ...
                for line in some_src.lines() { self.push_line(&format!("  {line}")); }
                if let Some(r) = some_res.into_iter().next() {
                    self.push_line(&format!(
                        "  {out} = new ffi.Option<{payload_ty}>(1, {r});"
                    ));
                }
                self.push_line("} else {");
                for line in none_src.lines() { self.push_line(&format!("  {line}")); }
                self.push_line("}");
                results.push(out);
            }

            Instruction::ResultLower { result: r, results: wasm_results, .. } => {
                let r_ref = self.fresh();
                self.push_line(&format!("const {r_ref} = {};", operands[0]));
                let result_locals: Vec<String> =
                    wasm_results.iter().map(|_| self.fresh()).collect();
                ...
                if r.ok.is_some() {
                    self.push_line(&format!(
                        "  const __variant_payload__ = {r_ref}.okValue;"
                    ));
                }
                for line in ok_src.lines() { self.push_line(&format!("  {line}")); }
                ...
                if r.err.is_some() {
                    self.push_line(&format!(
                        "  const __variant_payload__ = {r_ref}.errValue;"
                    ));
                }
                for line in err_src.lines() { self.push_line(&format!("  {line}")); }
            ...
            Instruction::ResultLift { result: r, .. } => {
                ...
                for line in ok_src.lines() { self.push_line(&format!("  {line}")); }
                let ok_arg = ok_res.into_iter().next().unwrap_or_else(|| ok_default.clone());
                self.push_line(&format!(
                    "  {out} = new ffi.Result<{ok_ty}, {err_ty}>(0, {ok_arg}, {err_default});"
                ));
                self.push_line("} else {");
                for line in err_src.lines() { self.push_line(&format!("  {line}")); }
                let err_arg = err_res.into_iter().next().unwrap_or_else(|| err_default.clone());
                self.push_line(&format!(
            ...
            Instruction::FixedLengthListLift { size, element, .. } => {
                ...
                let arr = self.fresh();
                self.push_line(&format!(
                    "const {arr} = new StaticArray<{t}>({size});"
                ));
            ...
            Instruction::FixedLengthListLowerToMemory { size, element, .. } => {
                ...
                let (block_src, _) =
                    self.blocks.pop().expect("FixedLengthListLowerToMemory expects a block");
                for i in 0..*size {
                    let base = self.fresh();
                    let elem = self.fresh();
                    self.push_line(&format!(
    ...
    fn return_pointer(&mut self, size: ArchitectureSize, align: Alignment) -> Self::Operand {
        ...
        let align_bytes = align.align_wasm32() as usize;
    ...
    fn is_list_canonical(&self, _resolve: &Resolve, element: &Type) -> bool {
        matches!(
            ...
            Type::U8 | Type::S8 | Type::U16 | Type::S16 | Type::U32 | Type::S32
                | Type::U64 | Type::S64 | Type::F32 | Type::F64
        )
    }
}
